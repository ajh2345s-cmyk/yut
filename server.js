...
function createApp({adminCode=process.env.ADMIN_CODE||'gmltn',dataFile=process.env.DATA_FILE||path.join(__dirname,'data','state.json'),now=Date.now,tickMs=250}={}){
 ...
 const IDLE_MS=60*60*1000,REVEAL_MS=2300;
 const rooms=new Map(),sessions=new Map(),attempts=new Map();let storageError=false,closing=false,lastCheckpoint=now();
 if(dataFile){let loaded=false;for(const file of[dataFile,dataFile+'.bak']){if(!fs.existsSync(file))continue;try{const data=JSON.parse(fs.readFileSync(file,'utf8'));if(data.version!==2)throw Error('저장 버전이 다릅니다.');for(const r of data.rooms){r.mode ||= 'tournament';r.emptySince ??= data.savedAt;r.teacherSocket=null;for(const p of Object.values(r.players)){p.socket=null;p.active=false;}for(const m of r.rounds.flat())if(m.clock){m.clock.remaining=m.clock.deadline?Math.max(0,m.clock.deadline-data.savedAt):m.clock.remaining;m.clock.deadline=null;}rooms.set(r.code,r);}for(const [k,v]of data.sessions)sessions.set(k,v);loaded=true;break;}catch(e){console.error('저장 파일 읽기 실패:',file,e.message);}}if(!loaded&&(fs.existsSync(dataFile)||fs.existsSync(dataFile+'.bak')))throw Error('저장 파일 복구가 필요합니다. 원본 데이터를 확인하세요.');}
 ...
 function hasConnections(r){return !!r.teacherSocket||Object.values(r.players).some(p=>!!p.socket);}
 function prune(){let changed=false;for(const [code,r]of rooms){if(hasConnections(r)){r.emptySince=null;continue;}if(r.emptySince===null||r.emptySince===undefined)r.emptySince=now();if(now()-r.emptySince>=IDLE_MS){rooms.delete(code);for(const [token,session]of sessions)if(session.room===code)sessions.delete(token);changed=true;}}if(changed)save();}
 prune();
 function reveal(r,m){m.revealUntil=now()+REVEAL_MS;m.game.last.eventId=crypto.randomUUID();m.game.last.by=m.game.players[m.game.turn];}
 function block(r,m){if(m.revealUntil>now())return '윷 결과 확인 중';if(storageError)return '저장 오류';if(r.paused)return '선생님 일시정지';if(!r.teacherSocket)return '선생님 재접속 대기';if(m.players.some(id=>!r.players[id]?.socket||!r.players[id]?.active))return '참가자 재접속 대기';return '';}
 ...
 function attach(s,r,role,playerId,token){const old=role==='teacher'?r.teacherSocket:r.players[playerId]?.socket;if(role==='student'&&!r.players[playerId])throw Error('이 참가 자리는 종료됐어요.');r.emptySince=null;s.data={room:r.code,role,playerId,token,view:null};if(role==='teacher')r.teacherSocket=s.id;else {r.players[playerId].socket=s.id;r.players[playerId].active=true;}if(old&&old!==s.id){const previous=io.sockets.sockets.get(old);if(previous){previous.emit('replaced');previous.disconnect(true);}}const own=r.rounds[r.round]?.find(m=>m.players.includes(playerId));s.data.view=own?.id||null;return {token,role,playerId,code:r.code};}
 ...
 function finish(r,m,winner,reason){m.winner=winner;m.status='finished';m.reason=reason;m.clock=null;if(m.game){m.game.winner=winner;m.game.phase='finished';m.game.revision++;}if(r.rounds[r.round].every(x=>x.winner!==null)){r.phase=r.round===r.rounds.length-1?'finished':'between';if(r.phase==='finished')r.champion=r.mode==='manual'?null:winner;}}
 function applyPlay(r,m,d){if(d.action==='throw'){G.roll(m.game);reveal(r,m);}else if(d.action==='move')G.move(m.game,d.piece,d.resultId,d.shortcut);else throw Error('잘못된 요청이에요.');if(m.game.winner!==null)finish(r,m,m.game.winner,'두 말 도착');else resetClock(r,m);}
 ...
 io.on('connection',socket=>{
  ...
  on('enter',d=>{
   prune();if(socket.data.room)throw Error('이미 입장했어요.');let r,role,playerId=null;
   if(d.role==='teacher'){const ip=socket.handshake.address;let a=attempts.get(ip);if(!a||now()-a.time>60000){a={time:now(),n:0};attempts.set(ip,a);}if(++a.n>10)throw Error('1분 후 다시 시도해 주세요.');if(!verify(d.password))throw Error('선생님 코드가 맞지 않아요.');role='teacher';if(d.code){r=rooms.get(String(d.code));if(!r)throw Error('교실 번호를 확인해 주세요.');if(r.teacherSocket)throw Error('이미 선생님이 접속 중이에요.');}else{if(rooms.size>=100)throw Error('교실이 가득 찼어요.');let code;do{code=String(crypto.randomInt(100000,1000000));}while(rooms.has(code));r={code,mode:'tournament',emptySince:null,teacherSocket:null,players:{},rounds:[],round:-1,phase:'lobby',paused:false,champion:null,settings:{throwSeconds:15,moveSeconds:30},touched:now()};rooms.set(code,r);}}
   else{role='student';r=rooms.get(String(d.code));if(!r)throw Error('교실 번호를 확인해 주세요.');const name=typeof d.name==='string'?d.name.trim().normalize('NFC'):'';if(!name||name.length>6||/[\x00-\x1f<>]/.test(name))throw Error('이름은 1~6자로 입력해 주세요.');if(Object.values(r.players).some(p=>p.name.normalize('NFKC').toLowerCase()===name.normalize('NFKC').toLowerCase()))throw Error('같은 이름이 있어요. 원래 기기로 접속하거나 선생님에게 복구를 요청하세요.');if(Object.keys(r.players).length>=100)throw Error('교실이 가득 찼어요.');playerId=crypto.randomUUID();r.players[playerId]={id:playerId,name,socket:null,active:true,eligible:r.phase==='lobby'};}
   const result=establish(socket,r,role,playerId);push(r);return result;
  });
  on('resume',d=>{prune();if(socket.data.room)throw Error('이미 입장했어요.');const s=sessions.get(d.token),r=s&&rooms.get(s.room);if(!r)throw Error('교실이 만료되었거나 참가 정보가 없어요. 선생님의 새 교실로 입장해 주세요.');const result=attach(socket,r,s.role,s.playerId,d.token);push(r);return result;});
  ...
  on('admin',d=>{
   const r=teacher();
   if(d.action==='draw'){if(r.mode!=='tournament')throw Error('대진 모드에서만 자동 대진을 만들 수 있어요.');if(r.phase!=='lobby')throw Error('이미 대진을 확정했어요.');r.rounds=G.bracket(Object.values(r.players).filter(p=>p.eligible&&p.socket).map(p=>p.id));r.round=0;r.phase='ready';}
   else if(d.action==='prepare'){if(r.mode==='manual')throw Error('자유 매치는 선생님이 새 경기를 지정해 주세요.');if(r.phase!=='between')throw Error('모든 경기가 끝나야 해요.');const previous=r.rounds[r.round++];r.rounds[r.round].forEach((m,i)=>m.players=[previous[i*2].winner,previous[i*2+1].winner]);r.phase='ready';}
   ...
   else if(d.action==='mode'){if(r.phase!=='lobby'||r.rounds.length)throw Error('새 대회 준비 후 모드를 바꿔 주세요.');if(!['tournament','manual'].includes(d.mode))throw Error('올바른 모드를 골라 주세요.');r.mode=d.mode;}
   else if(d.action==='manual'){
    if(r.mode!=='manual'||!['lobby','finished'].includes(r.phase))throw Error('현재 경기가 모두 끝난 뒤 자유 매치를 지정하세요.');
    if(!Array.isArray(d.pairs)||d.pairs.length<1||d.pairs.length>32)throw Error('경기를 1~32개 지정해 주세요.');
    const used=new Set();for(const pair of d.pairs){if(!Array.isArray(pair)||pair.length!==2)throw Error('경기마다 두 학생을 골라 주세요.');for(const id of pair){if(typeof id!=='string'||!r.players[id])throw Error('교실에 있는 학생을 골라 주세요.');if(used.has(id))throw Error('한 학생을 여러 경기나 양쪽에 중복 배정할 수 없어요.');used.add(id);}}
    const batch=r.rounds.length;
    r.rounds.push(d.pairs.map((players,i)=>({id:`f${batch+1}m${i+1}`,players:[...players],status:'pending',winner:null,game:null})));
    r.round=batch;r.phase='ready';r.paused=false;r.champion=null;
   }
  ...
  socket.on('disconnect',()=>{if(closing)return;const r=rooms.get(socket.data.room);if(!r)return;if(r.teacherSocket===socket.id)r.teacherSocket=null;const p=r.players[socket.data.playerId];if(p?.socket===socket.id){p.socket=null;p.active=false;}if(!hasConnections(r)&&r.emptySince===null)r.emptySince=now();try{push(r);}catch(e){console.error(e.message);}});
 });
 ...
 async function tick(){if(closing)return;prune();const work=[];for(const r of rooms.values())for(const m of r.rounds[r.round]||[]){if(m.status!=='playing')continue;if(m.revealUntil&&m.revealUntil<=now()){m.revealUntil=null;push(r);}syncClock(r,m);if(!block(r,m)&&m.clock?.deadline!==null&&m.clock.deadline<=now()&&!confirmations.has(m)){
  confirmations.add(m);const revision=m.game.revision,deadline=m.clock.deadline;
  work.push((async()=>{try{const online=await confirmConnected(r,m);if(closing)return;if(online&&!block(r,m)&&m.status==='playing'&&m.game.revision===revision&&m.clock?.deadline===deadline){const wasThrow=m.game.phase==='throw';G.auto(m.game);if(wasThrow)reveal(r,m);if(m.game.winner!==null)finish(r,m,m.game.winner,'두 말 도착');else resetClock(r,m);}push(r);}finally{confirmations.delete(m);}})());
 ...
 async function close(){closing=true;clearInterval(timer);for(const r of rooms.values()){if(hasConnections(r)||r.emptySince===null)r.emptySince=now();r.teacherSocket=null;for(const p of Object.values(r.players)){p.socket=null;p.active=false;}synchronize(r);}save();await new Promise(resolve=>io.close(resolve));}
